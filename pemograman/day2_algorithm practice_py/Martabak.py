...
def martabak_program():
    ...
    #Pilihan untuk masuk ke aplikasi atau keluar aplikasi
    ulang_arr = []
    ulang_arr.append(str(input("Membeli y/n: ")))

    if ulang_arr[0] == "n" or ulang_arr[0] == "N":
        exit()
    #Mengulang aplikasi sebanyak Permintaan
    while ulang_arr[0] == "y" or ulang_arr[0] == "Y" :
        ulang_arr.clear()
        ...
        while j < banyak_pesanan:
            jumlah_bayar = jumlah_bayar + harga[j] + harga_topping[j]
            print('%i   %s    %s     %i' % (j+1, jenis_martabak[j], topping[j], harga [j] + harga_topping[j]))
            j = j + 1
        ...
        while True:
            kembalian = int(bayar) - int(total_bayar)
            try:
                val = int(kembalian)
                if int(kembalian) < 0 :
                    print("uang kurang")
                    ulang = str(input("ulang y/n:"))
                    if str(ulang) == "y" or str(ulang) == "Y":
        ...
        #memasukan Log belanjaan ke dalam file txt untuk dokumentasi
        ...
        file.write(" (kembalian)Rp.")
        file.write(str('{:.0f}'.format(kembalian)))
        file.write("\n")

        file.close()
        #Pilihan untuk masuk ke aplikasi atau keluar aplikasi
        kembali = str(input("kembali y/n:"))
        if str(kembali) == "y" or str(kembali) == "Y":
            return martabak_program()
        else:
            exit()
